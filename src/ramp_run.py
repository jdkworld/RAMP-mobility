...
for country in countries:
    ...
    #inputfile = f'{ROOT}/input/{continent}/{c}'
    simulation_name = ''
    inputcountry = Inputs(country, continent)
    year = inputcountry.year

    ## If simulating the RES Integration charging strategy, a file with the residual load curve should be included in the folder
    try:
        inputfile_residual_load = f'{ROOT}/database/residual_load/residual_load_{inputcountry.country_b}.csv'
    ...
    (Profiles_list, Usage_list, User_list, Profiles_user_list, dummy_days
     ) = Stochastic_Process_Mobility(inputcountry, country, year, full_year)
    ...
    Profiles_utc = pp.Time_correction(Profiles_df, inputcountry.country_b, year)
    Usage_utc = pp.Time_correction(Usage_df, inputcountry.country_b, year)
    ...
    temp_profile = pp.temp_import(inputcountry.country_b, year, inputfile_temp) #Import temperature profiles, change the default path to the custom one
